...
public class Distance {
     ...
     * @param options String designation of unit type
     *              Creates a distance object with data
     */
    public Distance(Place orig, Place dest, Option options){
        origin = orig;
        destination = dest;
     ...
     */
    public static int calcDistance(double lat1, double long1, double  lat2, double long2, String units, double radius){
        if(units.equals("miles")) {
            ...
            return (int) Math.round((radius * getDeltaSigma(lat1, long1, lat2, long2)));
        }else {
            return -1;
        }
    }
    ...
    public void setDistance(){
        this.distance = calcDistance(this.origin.latitude, this.origin.longitude, this.destination.latitude, this.destination.longitude, this.options.units, this.options.unitRadius);
    }
    /**
    ...
    public String toString(){
        return String.format("Origin: latitude: %f, longitude: %f, name: %s, Destination: latitude: %f, longitude: %f, name: %s, Units: %s, Distance: %d",
                ...
                this.options.units, this.distance);
    }
}
